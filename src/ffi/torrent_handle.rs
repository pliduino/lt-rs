...
pub(crate) mod ffi {
    ...
    unsafe extern "C++" {
        ...
        fn torrent_handle_force_reannounce(handle: &torrent_handle);
        ...
        // rename
        fn torrent_handle_rename_file(handle: &torrent_handle, file_index: i32, new_name: &str);

        // queue position
        fn torrent_handle_queue_position(handle: &torrent_handle) -> i32;
        fn torrent_handle_queue_position_up(handle: &torrent_handle);
        fn torrent_handle_queue_position_down(handle: &torrent_handle);
        fn torrent_handle_queue_position_top(handle: &torrent_handle);
        fn torrent_handle_queue_position_bottom(handle: &torrent_handle);

        // web seeds
        fn torrent_handle_add_url_seed(handle: &torrent_handle, url: &str);
        fn torrent_handle_remove_url_seed(handle: &torrent_handle, url: &str);
        fn torrent_handle_url_seeds(handle: &torrent_handle) -> Vec<String>;

        // piece priorities
        fn torrent_handle_piece_priority(handle: &torrent_handle, piece: i32) -> u8;
        fn torrent_handle_piece_priorities(handle: &torrent_handle) -> Vec<u8>;
        fn torrent_handle_prioritize_pieces(handle: &torrent_handle, priorities: &[u8]);
        fn torrent_handle_set_piece_deadline(handle: &torrent_handle, piece: i32, deadline_ms: i32);
        fn torrent_handle_reset_piece_deadline(handle: &torrent_handle, piece: i32);
        fn torrent_handle_clear_piece_deadlines(handle: &torrent_handle);

        // peer info (async, fires peer_info_alert)
        fn torrent_handle_post_peer_info(handle: &torrent_handle);
    }
}
