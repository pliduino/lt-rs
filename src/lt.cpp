...
#include "../target/cxxbridge/lt-rs/src/ffi.rs.h"
...
namespace libtorrent
{
    ...
    std::unique_ptr<lt::add_torrent_params> lt_parse_magnet_uri(rust::Str magnet_uri)
    {
    ...
    std::unique_ptr<lt::torrent_handle> lt_session_add_torrent(lt::session &session, const lt::add_torrent_params &params)
    {
    ...
    void lt_session_async_add_torrent(lt::session &session, const lt::add_torrent_params &params)
    {
    ...
    InfoHashCpp lt_add_torrent_params_info_hash(const lt::add_torrent_params &params)
    {
        const auto hash = params.info_hashes;
        return info_hash_t_to_info_hash_cpp(hash);
    }

    rust::Vec<uint8_t> lt_write_resume_data_buf(const lt::add_torrent_params &params)
    {
        ...
        // It's so dumb how there's no function to just convert a vector to a rust::Vec...
        // Instead of just copying the pointer and len from buf we have to manually loop like this
        // And due to the files being auto generated you can't simply add it
    ...
    std::unique_ptr<lt::add_torrent_params> lt_read_resume_data(rust::Slice<const uint8_t> buf)
    {
        ...
        return std::make_unique<lt::add_torrent_params>(lt::read_resume_data(buf_vec));
    }
    ...
    std::unique_ptr<torrent_status> lt_torrent_handle_status(const lt::torrent_handle &handle)
    {
        return std::make_unique<lt::torrent_status>(handle.status());
    }

    void lt_torrent_handle_save_resume_data(const lt::torrent_handle &handle, uint8_t flags)
    {
    ...
    std::unique_ptr<lt::torrent_handle> lt_torrent_status_handle(const lt::torrent_status &status)
    {
    ...
    std::unique_ptr<lt::torrent_handle> lt_alert_state_changed_handle(lt::state_changed_alert *alert)
    {
        return std::make_unique<lt::torrent_handle>(alert->handle);
    }
    ...
    std::unique_ptr<lt::torrent_handle> lt_alert_save_resume_data_handle(lt::save_resume_data_alert *alert)
    {
        return std::make_unique<lt::torrent_handle>(alert->handle);
    }
    std::unique_ptr<lt::add_torrent_params> lt_alert_save_resume_data_params(lt::save_resume_data_alert *alert)
    {
        return std::make_unique<lt::add_torrent_params>(alert->params);
    }

    // =======================  Save Resume Data Failed  =======================
    lt::save_resume_data_failed_alert *lt_alert_save_resume_data_failed_cast(lt::alert *alert)
    {
        return lt::alert_cast<lt::save_resume_data_failed_alert>(alert);
    }
    std::unique_ptr<lt::torrent_handle> lt_alert_save_resume_data_failed_handle(lt::save_resume_data_failed_alert *alert)
    {
        return std::make_unique<lt::torrent_handle>(alert->handle);
    }
    int lt_alert_save_resume_data_failed_error(lt::save_resume_data_failed_alert *alert)
    {
        if (alert->error)
            return alert->error.value();
        return 0;
