use crate::alerts::RequestDroppedAlert;
use crate::alerts::peer_alert::{Endpoint, PeerId};
use crate::ffi::alerts::request_dropped::ffi::{
    request_dropped_alert_get_block_index, request_dropped_alert_get_piece_index,
};
...
impl RequestDroppedAlert {
    ...
    pub fn block_index(&self) -> i32 {
        unsafe { request_dropped_alert_get_block_index(self.0) }
    }

    #[inline(always)]
    pub fn piece_index(&self) -> PieceIndex {
        unsafe { request_dropped_alert_get_piece_index(self.0) }
    }
}
