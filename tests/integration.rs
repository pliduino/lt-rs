...
fn create_torrent_single_file() {
    ...
    ct.set_piece_hashes(dir.to_str().unwrap()).unwrap();
...
#[test]
fn add_torrent_params_from_torrent_bytes_rejects_garbage() {
    let result = AddTorrentParams::from_torrent_bytes(b"not a torrent");
    assert!(result.is_err(), "garbage bytes should return Err");
}

#[test]
fn add_torrent_params_from_torrent_bytes_accepts_valid_torrent() {
    let dir = tmp_dir("torrent_bytes_src");
    let test_file = dir.join("hello.txt");
    fs::write(&test_file, b"hello world").unwrap();

    let mut fs_wrap = FileStorage::new();
    fs_wrap.add_files(test_file.to_str().unwrap());
    assert_eq!(fs_wrap.num_files(), 1);

    let mut ct = CreateTorrent::new(&mut fs_wrap, 0);
    let err = ct.set_piece_hashes(dir.to_str().unwrap());
    assert!(err.is_ok(), "set_piece_hashes error: {err:?}");
    let torrent_bytes = ct.generate();
    assert!(!torrent_bytes.is_empty());

    let result = AddTorrentParams::from_torrent_bytes(&torrent_bytes);
    assert!(result.is_ok(), "valid torrent bytes should parse: {result:?}");
    let params = result.unwrap();
    let ih = params.get_info_hash();
    assert!(ih.as_hex().len() >= 40, "expected valid info hash, got: {}", ih.as_hex());
}
